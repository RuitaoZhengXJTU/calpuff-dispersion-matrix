...
from .units import DEFAULT_NO2_MOLECULAR_WEIGHT_G_MOL, ppb_factor_array
from .case_builder import CalpuffCaseFactory, CalpuffDomain, load_csv_rows
from .config import load_case_config, mapping_value, project_path
from .calpost import parse_calpost_tseries
from .runtime import (
    _assert_success,
    _parse_start_utc,
    _resolve_executable,
...
DEFAULT_GENERATORS = ROOT / "data" / "examples" / "dc_md_va_20250623" / "generators.csv"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--case-config", "--case", dest="case_config", type=Path, default=None,
...
def _run_cases_by_source(
    ...
    def run_source(source_index: int) -> tuple[int, dict[int, float]]:
        ...
        for batch_index, receptor_rows in enumerate(active_batches):
            ...
            if args.resume:
                completed = _load_completed_response(case_dir)
            ...
            try:
                ...
                if args.dry_run:
                    ...
                    continue
                run_control_file(paths["calpuff_exe"], control, case_dir, case_dir / "CALPUFF_RUN.log", args.timeout_sec)
                _assert_success(case_dir / "CALPUFF.CON", case_dir / "CALPUFF_RUN.log")
                run_control_file(paths["calpost_exe"], calpost_control, case_dir, case_dir / "CALPOST_RUN.log", args.timeout_sec)
...
def _load_completed_response(case_dir: Path) -> dict[int, float] | None:
    """Return a resumable compact response only when its status is completed."""
    status_path = case_dir / "run_status.json"
    response_file = case_dir / "region_response.npz"
    if not status_path.exists() or not response_file.exists():
        return None
    ...
    return {
        int(key): float(value)
        for key, value in zip(saved["indices"], saved["values"], strict=True)
    }
