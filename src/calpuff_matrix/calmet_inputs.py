...
from .case_builder import CalpuffDomain
from .config import load_case_config, mapping_value
...
DEFAULT_STATIONS = ROOT / "data" / "examples" / "dc_md_va_20250623" / "stations.csv"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="optional case YAML; its calpuff_domain replaces the built-in example grid",
    )
    ...
    domain = CalpuffDomain.from_mapping(mapping_value(load_case_config(args.case_config), "calpuff_domain"))
    ...
    terrain_points = _grid_points(domain)
    ...
    _write_geo(output_dir / "GEO.DAT", terrain_values, args.land_use_code, domain)
    ...
    provenance = {
        ...
        "grid": {
            ...
            "projection": domain.projected_crs,
        },
        "terrain": {
            "field": "HGT:surface",
...
def _grid_points(domain: CalpuffDomain) -> list[tuple[float, float]]:
    inverse = Transformer.from_crs(domain.projected_crs, "EPSG:4326", always_xy=True)
    points: list[tuple[float, float]] = []
    for j in range(domain.ny):
        for i in range(domain.nx):
            x = (domain.xorig_km + (i + 0.5) * domain.dgrid_km) * 1000.0
            y = (domain.yorig_km + (j + 0.5) * domain.dgrid_km) * 1000.0
...
def _write_geo(path: Path, terrain_values: list[float], land_use_code: int, domain: CalpuffDomain) -> None:
    if len(terrain_values) != domain.nx * domain.ny:
    ...
    lines = [
        ...
        "0",
        domain.pmap,
        f"{domain.rlat0:<16}{domain.rlon0:<16}{domain.xlat1:<16}{domain.xlat2:<16}",
        f"{domain.feast_km:.1f} {domain.fnorth_km:.1f}",
        f"{domain.datum:<8}{'HRRR':<12}",
        f"{domain.nx:8d}{domain.ny:8d}{domain.xorig_km:12.3f}{domain.yorig_km:12.3f}{domain.dgrid_km:12.3f}{domain.dgrid_km:12.3f}",
        "KM",
        "0",
    ]
    lines.extend(" ".join([str(land_use_code)] * domain.nx) for _ in range(domain.ny))
    ...
    lines.append("1.0")
    for row in range(domain.ny):
        start = row * domain.nx
        lines.append(" ".join(f"{value:.2f}" for value in terrain_values[start : start + domain.nx]))
...
def _write_control(
    ...
    irtype: int,
    station_count: int,
    domain: CalpuffDomain,
) -> None:
    ns = ",".join(["0"] * domain.nz)
    nintr = ",".join(["9"] * domain.nz)
    zface = ",".join(f"{value:g}." for value in domain.zface_m)
    lines = [
        ...
        f"! NX={domain.nx} !",
        f"! NY={domain.ny} !",
        f"! DGRIDKM={domain.dgrid_km:.3f} !",
        f"! XORIGKM={domain.xorig_km:.3f} !",
        f"! YORIGKM={domain.yorig_km:.3f} !",
        f"! PMAP={domain.pmap} !",
        f"! DATUM={domain.datum} !",
        f"! FEAST={domain.feast_km:.1f} !",
        f"! FNORTH={domain.fnorth_km:.1f} !",
        "! IUTMZN=0 !",
        "! UTMHEM=N !",
        f"! XLAT1={domain.xlat1} !",
        f"! XLAT2={domain.xlat2} !",
        f"! RLON0={domain.rlon0} !",
        f"! RLAT0={domain.rlat0} !",
        f"! NZ={domain.nz} !",
        f"! ZFACE={zface} !",
        "!END!",
